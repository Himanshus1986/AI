...
FastAPI application with SQL Server (pyodbc only) and Ollama LLM
"""
import os
import json
import asyncio
...
# Database Connection using pyodbc only
class DatabaseManager:
    ...
    def _build_connection_string(self) -> str:
        """Build pyodbc connection string for SQL Server"""
        return (
        ...
        try:
            ...
            if query.strip().upper().startswith(('INSERT', 'UPDATE', 'DELETE')):
                ...
                if query.strip().upper().startswith('INSERT'):
                    # Return the inserted ID for INSERT operations
...
class SessionManager:
    ...
    def get_or_create_session(self, user_email: str) -> UserContext:
        ...
        if session_key not in self.in_memory_sessions:
            try:
                ...
                query = """
                SELECT ConversationContext, SelectedSystem 
                FROM UserSessions 
                WHERE UserEmail = ? AND LastActivity > ?
                """
                yesterday = datetime.utcnow() - timedelta(hours=24)
                result = self.db_manager.execute_query(query, (user_email, yesterday))

                if result and result[0][0]:
                    context_data = json.loads(result[0][0])
    ...
    def save_session(self, user_email: str, context: UserContext):
        ...
        try:
            ...
            check_query = "SELECT SessionID FROM UserSessions WHERE UserEmail = ?"
            existing = self.db_manager.execute_query(check_query, (user_email,))

            if existing:
                # Update existing session
                update_query = """
                UPDATE UserSessions 
                SET ConversationContext = ?, SelectedSystem = ?, LastActivity = GETDATE()
                WHERE UserEmail = ?
                """
                self.db_manager.execute_query(
                    update_query, 
                    (context_json, context.selected_system, user_email),
                    fetch=False
                )
            else:
                # Create new session
                insert_query = """
                INSERT INTO UserSessions (SessionID, UserEmail, ConversationContext, SelectedSystem)
                VALUES (?, ?, ?, ?)
                """
                session_id = str(uuid4())
                self.db_manager.execute_query(
                    insert_query,
                    (session_id, user_email, context_json, context.selected_system),
                    fetch=False
                )
...
class TimesheetService:
    ...
    def get_timesheet_entries(self, user_email: str, system: str, start_date: date = None, end_date: date = None) -> List[Dict]:
        ...
        try:
            ...
            query = f"""
            SELECT ID, EntryDate, ProjectCode, TaskCode, Hours, Description, Status
            FROM {table_name}
            WHERE UserEmail = ?
            """
            params = [user_email]
        ...
        try:
            ...
            for entry in entries:
                ...
                existing = self.db_manager.execute_query(
                    check_query, 
                    (user_email, entry.entry_date, entry.project_code)
                )
                ...
                if existing:
                    ...
                    update_query = f"""
                    UPDATE {table_name} 
                    SET Hours = ?, TaskCode = ?, Description = ?, UpdatedAt = GETDATE()
                    WHERE ID = ?
                    """
                    self.db_manager.execute_query(
                        update_query,
                        (entry.hours, entry.task_code, entry.description, existing[0][0]),
                        fetch=False
                    )
                    saved_entries.append(existing[0][0])
                else:
                    # Create new entry
                    insert_query = f"""
                    INSERT INTO {table_name} (UserEmail, EntryDate, ProjectCode, TaskCode, Hours, Description)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """
                    entry_id = self.db_manager.execute_query(
                        insert_query,
                        (user_email, entry.entry_date, entry.project_code, 
                         entry.task_code, entry.hours, entry.description),
                        fetch=False
                    )
                    if entry_id:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        test_query = "SELECT 1"
        db_manager.execute_scalar(test_query)
...
app = FastAPI(
    ...
    description="FastAPI backend for conversational timesheet management with Ollama LLM integration (pyodbc only)",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Conversational Timesheet Chatbot API (pyodbc)",
        "version": "1.0.0",
        "status": "running"
    }
